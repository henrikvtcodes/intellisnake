...
def calc_food_strategy(
    ai_pos: tuple[int, int], ai_direction: SnakeDirections, food_pos: tuple[int, int]
) -> SnakeDirections:
    ...
    if (food_pos != prev_food_pos) or (ai_pos == food_pos):
        ...
        current_food_strategy = FoodStrategy.NONE
        print(
            f"AI(Food): Food position changed - New food position is {food_pos} from {prev_food_pos}"
        )
        prev_food_pos = food_pos
    else:
        print(
            f"AI(Food): Food position unchanged. FOOD {food_pos} PREV_FOOD {prev_food_pos}"
        )
    ...
    if current_food_strategy == FoodStrategy.NONE:
        ...
        if ai_direction == SnakeDirections.UP or ai_direction == SnakeDirections.DOWN:
            ...
            if ai_x < food_x:
                print(
                    "AI(Food): Initial strategy: moving right to align horizontally with food"
                )
                current_food_strategy = FoodStrategy.HORIZONTAL_ALIGN_RIGHT
                return SnakeDirections.RIGHT
            elif ai_x > food_x:
                print(
                    "AI(Food): Initial strategy: moving left to align horizontally with food"
                )
                current_food_strategy = FoodStrategy.HORIZONTAL_ALIGN_LEFT
                return SnakeDirections.LEFT
        elif (
            ai_direction == SnakeDirections.LEFT
            or ai_direction == SnakeDirections.RIGHT
        ):
    ...
    # ------------ TEST VERTICAL ALIGNMENT ------------
        ...
        current_food_strategy == FoodStrategy.VERTICAL_ALIGN_UP
        or current_food_strategy == FoodStrategy.VERTICAL_ALIGN_DOWN
    ):
    ...
    # ------------ TEST HORIZONTAL ALIGNMENT ------------
        ...
        current_food_strategy == FoodStrategy.HORIZONTAL_ALIGN_LEFT
        or current_food_strategy == FoodStrategy.HORIZONTAL_ALIGN_RIGHT
    ):
...
def next_ai_move(
    ...
    if (ai_y <= Sizes.SNAKE_BLOCK) or (ai_y >= max_y):
        print(
            f"AI(V-Collision): Incoming Vertical (Top/Bottom Wall) Collision - Moving {ai_direction.value}"
        )
        if ai_direction == SnakeDirections.UP:
            ...
            if result == direction_for_food:
                ...
                print(
                    f"AI(V-Collision): (DN) Collision Avoidance, Moving {result.value}"
                )
            return result
        ...
        print(
            f"AI(H-Collision): Incoming Horizontal (Left/Right Wall) Wall Collision - Moving {ai_direction.value}"
        )
        if ai_direction == SnakeDirections.LEFT:
            ...
            if result == direction_for_food:
                ...
                print(
                    f"AI(H-Collision): (L) Collision Avoidance, Moving {result.value}"
                )
            return result
            ...
            if result == direction_for_food:
                ...
                print(
                    f"AI(H-Collision): (R) Collision Avoidance, Moving {result.value}"
                )
            return result
        else:
            pass
